...
function Group:update(dt)
	...
	for _, go in ipairs(self.game_objects) do
		-- if go.name ~= "tile" then debug:log(go.name) end
		go:update(dt)
	end
...
function Group:remove_from_mask(obj_uuid, mask)
	for i, obj in ipairs(self.masks[mask]) do
		if obj.id == obj_uuid then
			table.remove(self.masks[mask], i)
		end
	end
end

function Group:remove_from_layer(obj_uuid, layer)
	for i, obj in ipairs(self.layers[layer]) do
		if obj.id == obj_uuid then
